...
<%@page import="bean.adminXemKHthanhtoanbean"%>
...
<body>
    ...
    <ul class="nav navbar-nav">
    	...
    	<li class="active"><a href="adminKHthanhtoanController">Khách hàng đã thanh toán</a></li>
...
<table class="table">
  <tr>
    <td colspan="8" class="cart-title">Danh sách khách hàng đã thanh toán:</td>
  </tr>
  <tr>
    <th>Mã hóa đơn</th>
    <th>Mã khách hàng</th>
  ...
  <%
    ArrayList<adminXemKHthanhtoanbean> ds = (ArrayList<adminXemKHthanhtoanbean>) request.getAttribute("dsdatt");
    for (adminXemKHthanhtoanbean tt : ds) {
	%>
	<tr>
    	...
    	<td><%= tt.getHoten() %></td>
    	<td><%= tt.getSodt() %></td>
    	<td><%= tt.getDiachimua() %></td>
    	<td><%= tt.getNgaymua() %></td>
    	<td style="color: teal"><%= tt.getTongtien() %> VND</td>
    	<td style="color: blue"><strong>Đã thanh toán</strong></td>
	</tr>
    <tr>
   	</tr>
  <%}%>
